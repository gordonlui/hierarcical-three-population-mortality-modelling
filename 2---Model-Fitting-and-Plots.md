...
``` math
\operatorname{logit}(q_{x,t}) = \kappa_{t,1} + (x - \bar{x})\kappa_{t,2} = \eta_{x,t}
```

M6:

``` math
\operatorname{logit}(q_{x,t}) = \kappa_{t,1} + (x - \bar{x})\kappa_{t,2} + \gamma_{t-x} = \eta_{x,t}
```

M7:

``` math
\operatorname{logit}(q_{x,t}) = \kappa_{t,1} + (x - \bar{x})\kappa_{t,2} + \left((x - \bar{x})^2 - \sigma^2\right)\kappa_{t,3} + \gamma_{t-x} = \eta_{x,t}
```
...
``` math
\operatorname{logit}(q^{R1}_{x,t}) = \eta^{R1}_{x,t}
```

Second Reference Population (R2):

``` math
\operatorname{logit}(q^{R2}_{x,t}) - \operatorname{logit}(q^{R1}_{x,t}) = \eta^{R2}_{x,t}
```

Book Population (B):

``` math
\operatorname{logit}(q^{B}_{x,t}) - \operatorname{logit}(q^{R2}_{x,t}) = \eta^{B}_{x,t}
```
